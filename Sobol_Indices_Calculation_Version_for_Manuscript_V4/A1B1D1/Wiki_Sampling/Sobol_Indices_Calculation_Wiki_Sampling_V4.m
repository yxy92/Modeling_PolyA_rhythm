...
%% parameter
output_dir = 'D:\CircadianRhythmicity\2019Fall\Sobol_Indices_Calculation_Version_for_Manuscript_V4\A1B1D1\Wiki_Sampling\Table\';
...
A= Sampling(Input_Number,Sample_Size,Method,mu_deA,mu_dgrd,sigma);
B= Sampling(Input_Number,Sample_Size,Method,mu_deA,mu_dgrd,sigma);
C= Sampling(Input_Number,Sample_Size,Method,mu_deA,mu_dgrd,sigma);
...
function  X = Sampling(Input_Number,Sample_Size,Method,mu_deA,mu_dgrd,sigma)
% X --- a parameter set 
...
for i=1:Sample_Size
    ...
    X(i,6) = log(2)/(10^icdf(hl_dgrd,X(i,6))); % Kdgrd       
    X(i,7) = X(i,7); % Adgrd
    X(i,8) = 24*X(i,8); % Pdgrd
end
