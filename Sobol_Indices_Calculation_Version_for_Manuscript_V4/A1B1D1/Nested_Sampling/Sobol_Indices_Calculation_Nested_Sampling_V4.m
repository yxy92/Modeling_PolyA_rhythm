...
output_dir = 'D:\CircadianRhythmicity\2019Fall\Sobol_Indices_Calculation_Version_for_Manuscript_V4\A1B1D1\Nested_Sampling\Table\';
...
[Pi, Pni,Ptot] = Transform_Cube_to_Parameter(Xi, Xni,Xtot,mu_deA,mu_dgrd,sigma);
...
function [Pi, Pni, Ptot] = Transform_Cube_to_Parameter(Xi, Xni, Xtot,mu_deA,mu_dgrd,sigma)
...
for i=1:dim_i(3) % Number of input
    for j=1:dim_i(1) % Number Group * Number of sampling within each group
        ...
        Pi(j,6,i) = log(2)/(10^icdf(hl_dgrd,Pi(j,6,i))); % Kdgrd       
        Pi(j,7,i) = Pi(j,7,i); % Adgrd
        Pi(j,8,i) = 24*Pi(j,8,i); % Pdgrd
        ...
        Pni(j,6,i) = log(2)/(10^icdf(hl_dgrd,Pni(j,6,i))); % Kdgrd       
        Pni(j,7,i) = Pni(j,7,i); % Adgrd
        Pni(j,8,i) = 24*Pni(j,8,i); % Pdgrd
...
for i=1:dim_tot(1)
    ...
    Ptot(i,6) = log(2)/(10^icdf(hl_dgrd,Ptot(i,6))); % Kdgrd       
    Ptot(i,7) = Ptot(i,7); % Adgrd
    Ptot(i,8) = 24*Ptot(i,8); % Pdgrd
...
dxdt(1) = Ktrsc*(1+Atrsc*cos(omg*(t-Ptrsc)))-KdeA*(1+AdeA*cos(omg*(t-PdeA)))*L;
dxdt(2) = KdeA*(1+AdeA*cos(omg*(t-PdeA)))*L - Kdgrd*(1+Adgrd*cos(omg*(t-Pdgrd)))*S;
