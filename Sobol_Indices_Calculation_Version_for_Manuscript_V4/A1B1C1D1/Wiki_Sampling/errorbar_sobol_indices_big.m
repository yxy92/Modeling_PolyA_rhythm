...
for i=1:length(outputQuantity)
    ...
    y = mean(tmp);
    err = std(tmp);
    errorbar(x,y,err,'LineWidth',2,'LineStyle','None','Marker','.','MarkerSize',20,'CapSize',14);
    ...
    saveas(gcf,strcat(output_dir,outputQuantityFile{i},single_figure));
end
...
for i=1:length(outputQuantity)
    ...
    title(outputQuantity{i});
    saveas(gcf,strcat(output_dir,outputQuantityFile{i},total_figure));
end
